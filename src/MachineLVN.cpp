...
bool MachineLVN::skip(MachineInstruction *inst)
{
    ...
    for (auto def : inst->getDef())
    {
        if (def->isReg())
        {
            if (def->isFReg() && def->getReg() < 16)
                return true;
            if (!def->isFReg() && def->getReg() < 4)
                return true;
        }
    }
    ...
    if (inst->isBinary() || (inst->isLoad() && (inst->getUse()[0]->isImm() || inst->getUse()[0]->isLabel())) || (inst->isMovClass() && !inst->isVMov()))
        return false;
    return true;
}
